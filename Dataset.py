...
class DogDataset(Dataset):
    ...
    def get_dataset_box(self, idx) -> (str, Sequence[int], str):
        ...
        print("image size:", Image.open(image_file).size)
    ...
    @staticmethod
    def draw_box(image_file: str, box: Tensor,
                 label: List[str], color: Union[str, List[str]] = 'yellow'):
        ...
        def auto_adapt_font_size(pos):
            ...
            return predicted_size \
                if predicted_size < good_font_size["font_size"] else good_font_size["font_size"]
    ...
    @staticmethod
    def convert(size: Union[tuple, list],
                bbox: Union[Tensor, Sequence[Union[int, float]]],
                only_norm: bool = False, denormalize: bool = False):
        if denormalize:
            if only_norm:
                nx1, ny1 = bbox[0] * size[0], bbox[1] * size[1]
                nx2, ny2 = bbox[3] * size[0], bbox[3] * size[1]
                return torch.tensor([nx1, ny1, nx2, ny2])

            # restore normalized bbox
            x, y = bbox[0] * size[0], bbox[1] * size[1]
            w, h = bbox[2] * size[0], bbox[3] * size[1]
            ...
            return torch.tensor([x_min, y_min, x_max, y_max]).int()
        else:
            if only_norm:
                nx1, ny1 = bbox[0] / size[0], bbox[1] / size[1]
                nx2, ny2 = bbox[3] / size[0], bbox[3] / size[1]
    ...
    def __getitem__(self, idx):
        ...
        if self.task == 'bounding_box':
            ano_file = os.path.join(self.pos_ano_path, self.ano_file_list[idx])
            # box = self.convert(image.size, self._get_xy(ano_file), only_norm=True)
            box = self.box_resize(self._get_xy(ano_file), image.size)
